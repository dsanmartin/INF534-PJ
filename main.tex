...
\begin{document}
        ...
        \begin{algorithmic}
          \Procedure{spreading}{$S^t, DW$}
            ...
            \For{$i=0$ to $N_{threads}-1$}
              ...
              \EndIf
              \State{Create Thread[$i$]}
              \State{Execute this portion of code in Thread[$i$]}
  ...
  \section{Evaluation}
    ...
    of discrete times used for the simulation. %To achieve a realistic simulation, $N=1500$ give us 
    %We assume the use of a square grid of size $N \times N$ and, defining the maximum number
    %of discrete times $T_{max}$ we estimate a computational complexity of $O(N^2)$. 
